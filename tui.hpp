...
struct List : Widget {
  ...
  template <typename Window> void scroll_up(Window &window, int factor = 1);
  template <typename Window> void scroll_down(Window &window, int factor = 1);
};
...
public:
  ...
  template <typename Widget> void draw_border(Widget widget) {
  ...
  // Draw title
  template <typename Widget> void draw_title(Widget widget) {
    short title_color =
  ...
  inline void clear() { 
    ...
    // The original header had this function empty. Without calling the global ncurses
    // clear function (::clear), the screen is never cleared between frames,
    // causing the text to draw on top of itself and become garbled.
    ::clear(); 
  };
...
template <> void Window::add(Paragraph paragraph) {
  // VERY NECESSARY CHANGE:
  // The original logic for drawing paragraphs was flawed. It treated the entire text
  // as a single block of characters and did not handle newline ('\n') characters.
  // This caused all lines of the matrix and instructions to try to draw on the same line,
  // resulting in the garbled, overwritten text seen in the bug report.
  // This new logic reads the text line by line and draws them separately,
  // which correctly renders the matrix and other information.
  ...
  // Use a stringstream to process the text line by line
  std::stringstream ss(paragraph.text);
  std::string line;
  int current_y = paragraph.y + 1; // Start drawing from the first line inside the border

  // Read each line from the stringstream until there are no more lines or no more space
  while (std::getline(ss, line, '\n') && current_y < paragraph.y + paragraph.height - 1) {
    for (int j = 0; j < line.length() && (paragraph.x + 1 + j) < (paragraph.x + paragraph.width - 1); ++j) {
      draw_char(paragraph.x + 1 + j, current_y, line[j], text_color);
    }
    current_y++; // Move to the next line for drawing
  }
}

template <> void Window::add(List list) {
  if (list.border == true) {
    draw_border(list);
  }
...
template <> void Window::add(BarChart bar_chart) {
...
template <> void Window::add(Gauge gauge) {
...
// Scroll up the list
template <> void List::scroll_up(Window &window, int factor) {
...
template <> void List::scroll_down(Window &window, int factor) {
  if (rows.size() > height - 2) {
    first_element =
